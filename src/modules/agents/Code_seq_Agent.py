...
class Code_seq_Agent(nn.Module):
    def __init__(self, input_shape, args):
        super(Code_seq_Agent, self).__init__()
        self.args = args
        ...
        self.fc3 = nn.Linear(input_shape + args.n_actions + args.intent_dim, args.rnn_hidden_dim)
    ...
    def init_decoder_hidden(self):
        return self.fc3.weight.new(1, self.args.rnn_hidden_dim).zero_()
    ...
    def intent_infer(self, input, last_action, intent, hidden):
        # hidden = self.init_decoder_hidden().expand(intent.shape[0], -1)
        output_seq = [] 
        for i in range(3):
            if i == 0:
                input_seq = last_action
            combined_input = th.cat((input[i], input_seq, intent), dim=1)
            x = F.relu(self.fc3(combined_input))
            hidden = self.gru(x, hidden)
            output = self.infer_net(hidden)
            output_seq.append(output)
            input_seq = output.detach()  # 更新输入为当前输出，实现自回归
