...
class StarCraft2Env(MultiAgentEnv):
    ...
    def _launch(self):
        ...
        step_x = self.map_x // grid_size
        step_y = self.map_y // grid_size
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                # 在当前四宫格内随机生成采样点
                sample_x = -(start_x - end_x)/2
                sample_y = -(start_y - end_y)/2
                
                # 存储采样点
                self.sample_points.append((sample_x, sample_y))
    ...
    def get_agent_action(self, a_id, action):
        ...
        if action == 0:
            ...
            if self.map_type == "MMM" and unit.unit_type == self.medivac_id:
                ...
                target_z = target_unit.pos.z
                delta_z = z - target_z
                if abs(delta_z) <= 0.5:
                    action_name = "attack"
                if delta_z > 0.5 and delta_z < 2.0:
                    action_name = "attack"
                if delta_z < -0.5 and delta_z > -2.0:
    ...
    def unit_max_shield(self, unit):
        ...
        if unit.unit_type == 73 or unit.unit_type == self.zealot_id or unit.unit_type == 1972:
            return 50  # Protoss's Zaelot
        if unit.unit_type == 4 or unit.unit_type == self.colossus_id or unit.unit_type == 1970:
    ...
    def get_obs_size_seg(self):
        ...
        start_point = move_feats
        end_ponit = move_feats + enemy_feats + ally_feats
        return start_point, end_ponit, self.n_enemies, nf_en, self.n_agents - 1, nf_al
    ...
    def get_unit_type_id(self, unit, ally):
        ...
        if ally:  # use new SC2 unit types
            ...
            if self.map_type == "stalkers_and_zealots":
            ...
            elif self.map_type == "stalkers_and_zealots_v1":
                type_id = 0
            elif self.map_type == "colossus_and_zealots":
                type_id = 0
                ...
                # baneling
                type_id = 0
            elif self.map_type == "overload_bane_1":
                # baneling
                type_id = 0
...
#         location_feats_len = len(self.sample_points)
        
#         move_feats = np.zeros(move_feats_len, dtype=np.float32)
#         location_feats = np.zeros((location_feats_len, 3), dtype=np.float32)
#         enemy_feats = np.zeros((self.n_enemies, nf_en), dtype=np.float32)
#         ally_feats = np.zeros((self.n_agents - 1, nf_al), dtype=np.float32)
#         own_feats = np.zeros(nf_own, dtype=np.float32)
...
#             y = unit.pos.y
#             sight_range = self.unit_sight_range(agent_id)
#             location_range = self.location_sight_range()
...
#                 item = self.sample_points[i]
#                 dist = self.distance(x, y, item[0], item[1])
#                 if dist < location_range:
#                     location_feats[i, 0] = dist / location_range  # distance
#                     location_feats[i, 1] = (
#                         item[0] - x
#                     ) / location_range  # relative X
#                     location_feats[i, 2] = (
#                         item[1] - y
#                     ) / location_range  # relative Y
...
#         return move_feats + enemy_feats + ally_feats + own_feats + location_feats
